    ...
    */
    public function actionIndex($sid = null)
    {
        ...
        if((Yii::app()->request->getPost('register')) && !$iTokenId){
          ...
          // $this->aRegisterErrors[] = gT("cheershelp.");
...
//	}
	  ...
	  // self::display($iSurveyId);
        ...
        // Display the page
//	$this->aRegisterErrors[] = gT("helpme.");
    ...
    */
    public function getRegisterErrors($iSurveyId){
        ...
        if($aFieldValue['sEmail']==""){
            $this->aRegisterErrors[]= gT("You cannot leave field blank. Please try again.");
    ...
    public function getRegisterForm($iSurveyId)
    {
       ...
       // $aData['sLastName'] = $aFieldValue['sLastName'];
        ...
        $aReplacement['REGISTERMESSAGE1'] = gT("You must be registered to compilete this survey");
        if($sStartDate=$this->getStartDate($iSurveyId))
            $aReplacement['REGISTERMESSAGE2'] = sprintf(gT("You may register for this survey but you have to wait for the %s before starting the survey."),$sStartDate)."<br />\n".gT("If you are comming from Amazon Mechnaical Turk please enter your Mechanical Turk Worker ID below so that you can be appropriately compensated. Otherwise create a username/pin for yourself.")."<br/> \n".gT("If you are unsure where to find your worker ID, the image below highlights where to find it on your dashboard, and the URL to get to your dashboard");
        else
...
https://bridge.apt.ri.cmu.edu/limesurvey/index.php?r=register/index&sid=791575
...
//            $aMail['subject'] = str_replace("@@{$key}URL@@", $url, $aMail['subject'])://bridge.apt.ri.cmu.edu/limesurvey/index.php?r=register/index&sid=791575;
...
//            }
//        }
        $myUrl=App()->createAbsoluteUrl("/survey/index/sid/{$iSurveyId}",array('lang'=>$sLanguage,'token'=>$sToken));
	    ...
	    $myUrl=App()->createAbsoluteUrl("/survey/index/sid/{$iSurveyId}",array('lang'=>$sLanguage,'token'=>$sToken));
	        ...
	        . "<p>".gT("Thank you for registering to participate in this survey.Please Follow the link below to take the survey")."</p>\n"
		. "<p><a href='{$myUrl}'>".gT("Go To Survey")."</a></p>\n"
    ...
    */
    public function getTokenId($iSurveyId)
    {
        ...
        if ($oToken)
         {
           // $this->aRegisterErrors[]=gT("shoop");

	   // self::display($iSurveyId);

            if($oToken->usesleft<1 && $aSurveyInfo['alloweditaftercompletion']!='Y')
            {
                $this->aRegisterErrors[]=gT("The Mechanical Turk/User ID you entered has already been registered and the survey has been completed.");
            }
            ...
            {
                $this->sMailMessage=gT("The Mechanical Turk/User ID you entered has already been registered");
                return $oToken->tid;
            }
        }
        else
	  ...
	  //  $this->aRegisterErrors[]=gT("shoop");
           ...
           // self::display($iSurveyId);
            ...
            $oToken->firstname = $aFieldValue['sEmail'];
            $oToken->lastname = $aFieldValue['sEmail'];
	   ...
	   // $this->aRegisterErrors[]=gT("shoop {$oToken->tid}");
